...
pixel of the same color as the starting pixel, plus any pixels connected 4-directionally to those pixels (also with the
...
class Solution:
    def floodFill(self, image: List[List[int]], sr: int, sc: int, newColor: int) -> List[List[int]]:
        if (newColor != image[sr][sc]):
            self.floodImage(image, sr, sc, image[sr][sc], newColor)
        return image

    def floodImage(self, image, x, y, oldValue, newValue):
        if x < 0 or y < 0 or x > len(image) - 1 or y > len(image[0]) - 1 or image[x][y] != oldValue:
            return
        image[x][y] = newValue
        self.floodImage(image, x - 1, y, oldValue, newValue)
        self.floodImage(image, x + 1, y, oldValue, newValue)
        self.floodImage(image, x, y - 1, oldValue, newValue)
        self.floodImage(image, x, y + 1, oldValue, newValue)